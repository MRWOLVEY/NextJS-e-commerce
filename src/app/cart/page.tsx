...
const Cart = () => {
  const { state, dispatch, actions } = useContext(ShopContext) as unknown as {
    state: { cart: CartState; cartProductsCount: number };
    dispatch: Function;
    actions: any;
  };
  ...
  const cartData = useMemo(() => {
    ...
    const result: any[] = [];
    Object.keys(state.cart).forEach((key: string) => {
      Object.keys(state.cart[key]).forEach((size: string) => {
        if (size !== "total") {
    ...
    return result;
  }, [state.cart, products, productsLoading]);
                  ...
                  e.target.value === "0" || e.target.value === ""
                    ? ((e.target.value = "1"),
                      dispatch({
                        ...
                        payload: { id: item.id, size: item.size, quantity: 1 },
                      }))
                    : dispatch({
                        type: actions.updateQuantity,
                        payload: {
